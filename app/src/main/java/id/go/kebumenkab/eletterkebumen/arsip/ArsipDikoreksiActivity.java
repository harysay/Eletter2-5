...
import id.go.kebumenkab.eletterkebumen.adapter.pns.ArsipDikoreksiAdapter;
...
import id.go.kebumenkab.eletterkebumen.model.ArsipDikoreksiData;
...
public class ArsipDikoreksiActivity extends AppCompatActivity  implements
        SwipeRefreshLayout.OnRefreshListener, ArsipDikoreksiAdapter.MessageAdapterListener{

    private List<ArsipDikoreksiData> arsips = new ArrayList<>();
    private RecyclerView recyclerView;
    public ArsipDikoreksiAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        logger = new Logger();
        notifikasiDialog = new NotifikasiDialog(getApplicationContext(), ArsipDikoreksiActivity.this);
        ...
        mAdapter = new ArsipDikoreksiAdapter(getApplicationContext(), arsips,
                ArsipDikoreksiActivity.this);
    ...
    private void getData() {
        ...
        String jenisArsip = getIntent().getStringExtra(Tag.TAG_JENISARSIP);

        Call<ResultArsipDikoreksi> call = apiService.getArsipDikoreksiLengkap(token,String.valueOf(currentPage));

        if(jenisArsip.equalsIgnoreCase(Tag.TAG_KOREKSI)){
            call = apiService.getArsipDikoreksiLengkap(token, String.valueOf(currentPage));
        }
        if(call != null) {
            call.enqueue(new Callback<ResultArsipDikoreksi>() {
                @Override
                public void onResponse(Call<ResultArsipDikoreksi> call, Response<ResultArsipDikoreksi> response) {
                    final ResultArsipDikoreksi result = response.body();
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (result != null) {
                                    ...
                                    List<ArsipDikoreksiData> todos = result.getData();
                                    ...
                                    if (todos.size() == 0) {
                                     ...
                                     //   totalPage = result.getTotalPage();
                                        ...
                                        for (ArsipDikoreksiData data : todos) {
                                            arsips.add(data);
                                        }
                ...
                @Override
                public void onFailure(Call<ResultArsipDikoreksi> call, Throwable t) {
                    swipeRefreshLayout.setRefreshing(false);
    ...
    private void getDataNext(int page) {
        ...
        Call<ResultArsipDikoreksi> call = apiService.getArsipDikoreksiLengkap(token, String.valueOf(page));
        ...
        if(call != null) {
            call.enqueue(new Callback<ResultArsipDikoreksi>() {
                ...
                public void onResponse(Call<ResultArsipDikoreksi> call, Response<ResultArsipDikoreksi> response) {
                   ...
                   // logger.w8("diajukan", ResultArsipDikoreksi);
                    if(result  != null) {
    ...
    public void onMessageRowClicked(int position) {
        ...
        if (mAdapter.getSelectedItemCount() > 0) {
            ...
            ArsipDikoreksiData arsip = arsips.get(position);

            arsips.set(position, arsip);
            mAdapter.notifyDataSetChanged();

            Intent intentDetail = new Intent(getApplicationContext(), ArsipDikoreksiDetail.class);
