...
import id.go.kebumenkab.eletterkebumen.adapter.pns.ArsipLainAdapter;
...
import id.go.kebumenkab.eletterkebumen.model.ArsipLainData;
...
public class ArsipLainActivity extends AppCompatActivity  implements
        SwipeRefreshLayout.OnRefreshListener, ArsipLainAdapter.MessageAdapterListener{

    private List<ArsipLainData> arsips = new ArrayList<>();
    private RecyclerView recyclerView;
    public  ArsipLainAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        logger = new Logger();
        notifikasiDialog = new NotifikasiDialog(getApplicationContext(), ArsipLainActivity.this);
        ...
        mAdapter = new ArsipLainAdapter(getApplicationContext(), arsips,
                ArsipLainActivity.this);
    ...
    private void getData() {
        ...
        Call<ResultArsipLain> call = apiService.getArsipLainLengkap(token,String.valueOf(currentPage));

        if(call != null) {
            call.enqueue(new Callback<ResultArsipLain>() {
                @Override
                public void onResponse(Call<ResultArsipLain> call, Response<ResultArsipLain> response) {
                    final ResultArsipLain result = response.body();
                    if(result  != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (result != null) {
                                    ...
                                    List<ArsipLainData> todos = result.getData();
                ...
                @Override
                public void onFailure(Call<ResultArsipLain> call, Throwable t) {
                    swipeRefreshLayout.setRefreshing(false);
    ...
    private void getDataNext(int page) {
        ...
        Call<ResultArsipLain> call = apiService.getArsipLainLengkap(token, String.valueOf(page));
    ...
    public void onMessageRowClicked(int position) {
        ...
        if (mAdapter.getSelectedItemCount() > 0) {
            ...
            ArsipLainData arsip = arsips.get(position);

            arsips.set(position, arsip);
            mAdapter.notifyDataSetChanged();

            Intent intentDetail = new Intent(getApplicationContext(), ArsipLainDetail.class);
