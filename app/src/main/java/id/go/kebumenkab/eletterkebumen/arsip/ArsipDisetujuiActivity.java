...
import id.go.kebumenkab.eletterkebumen.adapter.pns.ArsipDisetujuiAdapter;
...
public class ArsipDisetujuiActivity extends AppCompatActivity  implements
    ...
    private List<ArsipDisetujuiData> arsips = new ArrayList<>();
    ...
    private void getData() {
        ...
        Call<ResultArsipDisetujui> call = apiService.getArsipDisetujuiLengkap(token,String.valueOf(currentPage));

        if(call != null) {
            call.enqueue(new Callback<ResultArsipDisetujui>() {
                @Override
                public void onResponse(Call<ResultArsipDisetujui> call, Response<ResultArsipDisetujui> response) {
                    final ResultArsipDisetujui result = response.body();


                    logger.w9("disetujui", response);
                    if(result  != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (result != null) {
                                    ...
                                    List<ArsipDisetujuiData> todos = result.getData();
                                    ...
                                    if (todos.size() == 0) {
                                        ...
                                        for (ArsipDisetujuiData data : todos) {
                                            arsips.add(data);
                                        }
                ...
                @Override
                public void onFailure(Call<ResultArsipDisetujui> call, Throwable t) {
                    swipeRefreshLayout.setRefreshing(false);
    ...
    private void getDataNext(int page) {
        ...
        Call<ResultArsipDisetujui> call = apiService.getArsipDisetujuiLengkap(token, String.valueOf(page));
    ...
    public void onMessageRowClicked(int position) {
        ...
        if (mAdapter.getSelectedItemCount() > 0) {
            ...
            ArsipDisetujuiData arsip = arsips.get(position);
            ...
            Intent intentDetail = new Intent(getApplicationContext(), ArsipDisetujuiDetail.class);
