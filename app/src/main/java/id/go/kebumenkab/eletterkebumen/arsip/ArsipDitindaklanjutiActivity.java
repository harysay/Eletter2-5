...
import id.go.kebumenkab.eletterkebumen.adapter.pns.ArsipDitindaklanjutiAdapter;
...
import id.go.kebumenkab.eletterkebumen.model.ArsipDitindaklanjutiData;
...
public class ArsipDitindaklanjutiActivity extends AppCompatActivity  implements
        SwipeRefreshLayout.OnRefreshListener, ArsipDitindaklanjutiAdapter.MessageAdapterListener{

    private List<ArsipDitindaklanjutiData> arsips = new ArrayList<>();
    private RecyclerView recyclerView;
    public  ArsipDitindaklanjutiAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        logger = new Logger();
        notifikasiDialog = new NotifikasiDialog(getApplicationContext(), ArsipDitindaklanjutiActivity.this);
        ...
        mAdapter = new ArsipDitindaklanjutiAdapter(getApplicationContext(), arsips,
                ArsipDitindaklanjutiActivity.this);
    ...
    private void getData() {
        ...
        Call<ResultArsipDitindaklanjuti> call = apiService.getArsipDitindaklanjtiLengkap(token,String.valueOf(currentPage));

        if(call != null) {
            call.enqueue(new Callback<ResultArsipDitindaklanjuti>() {
                @Override
                public void onResponse(Call<ResultArsipDitindaklanjuti> call, Response<ResultArsipDitindaklanjuti> response) {
                    final ResultArsipDitindaklanjuti result = response.body();
                    if(result  != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (result != null) {
                                    ...
                                    List<ArsipDitindaklanjutiData> todos = result.getData();
                                    ...
                                    if (todos.size() == 0) {
                                        ...
                                        for (ArsipDitindaklanjutiData data : todos) {
                                            arsips.add(data);
                                        }
                ...
                @Override
                public void onFailure(Call<ResultArsipDitindaklanjuti> call, Throwable t) {
                    swipeRefreshLayout.setRefreshing(false);
    ...
    private void getDataNext(int page) {
        ...
        Call<ResultArsipDitindaklanjuti> call = apiService.getArsipDitindaklanjtiLengkap(token, String.valueOf(page));
    ...
    public void onMessageRowClicked(int position) {
        ...
        if (mAdapter.getSelectedItemCount() > 0) {
            ...
            ArsipDitindaklanjutiData arsip = arsips.get(position);

            arsips.set(position, arsip);
            mAdapter.notifyDataSetChanged();

            Intent intentDetail = new Intent(getApplicationContext(), ArsipDitindaklanjutiDetail.class);
